...
SITE_URL = "https://infinite2142.github.io/marketwatch/"
SITE_DESC = ("A daily read on markets, macro drivers and crash risk, alongside a running "
             "register of investible themes tracked from first sighting on the radar "
             "through to played-out. Global and UK lens. Research aid, not advice.")

def _preview_version():
    """Version the card image by the commit that last changed it, so a share preview
    refetches when the picture actually changes and not on every render. Falls back to
    the build date if git or the file is unavailable — a wrong version only costs a
    cache miss."""
    try:
        out = subprocess.run(["git", "log", "-1", "--format=%cs", "--", "preview.png"],
                             capture_output=True, text=True, check=True,
                             cwd=HERE, timeout=15).stdout.strip()
        if out:
            return out
    except Exception:
        pass
...
def head_meta(data, report_date):
    """The share card's words. The picture is regenerated by hand; these are rebuilt
    every render, so the text under a shared link is always the current day's."""
    meta = data.get("meta", {})
    crash = data.get("crash_risk", {})
    long_date = meta.get("report_date_long") or report_date
    comp, level = crash.get("composite"), (crash.get("level") or "").lower()
    title = "Market Watch — %s" % long_date
    if comp is not None:
        title = "Market Watch — crash risk %s of 100, %s · %s" % (comp, level, long_date)
    # The refresh label already opens with the day's headline, written for a reader.
    # Strip its "Analysis 27 Aug - " stamp, since the title carries the date.
    lead = re.sub(r"^Analysis\s+\d+\s+\w+\s*[-–—]\s*", "", meta.get("refresh_label", "")).strip()
    desc = SITE_DESC
    if len(lead) > 80:
        cut = lead[:230]
        if len(lead) > 230:                      # trim back to a sentence end
            dot = cut.rfind(". ")
            cut = (cut[:dot + 1] if dot > 110 else cut.rstrip() + "…")
        desc = cut
    img = SITE_URL + "preview.png?v=" + _preview_version()
...
def main():
    ...
    # 2) the head: canonical, Open Graph and Twitter tags, and a dated <title>
    share_title, tags = head_meta(data, report_date)
    html = html.replace("<!--HEADMETA-->", tags)
    html = re.sub(r"<title>.*?</title>",
                  "<title>%s</title>" % share_title.replace("\\", ""), html, count=1)
